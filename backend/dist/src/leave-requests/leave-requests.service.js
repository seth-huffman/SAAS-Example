...
let LeaveRequestsService = class LeaveRequestsService {
    ...
    async remove(id, requestUser) {
        ...
        if (requestUser.role === user_entity_1.UserRole.EMPLOYEE && lr.status !== leave_request_entity_1.LeaveStatus.PENDING) {
            throw new common_1.BadRequestException('Only pending requests can be deleted');
        }
