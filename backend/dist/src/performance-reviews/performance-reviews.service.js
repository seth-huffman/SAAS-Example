...
let PerformanceReviewsService = class PerformanceReviewsService {
    ...
    async deleteSelf(id, userId) {
        ...
        if (review.status !== performance_review_entity_1.ReviewStatus.SUBMITTED) {
            throw new common_1.ForbiddenException('Cannot delete a review that has already been approved');
        }
        await this.reviewRepo.remove(review);
    }
    async updateSelf(id, dto, userId) {
        const review = await this.findById(id);
        const emp = await this.empRepo.findOne({ where: { userId } });
        if (!emp || review.employeeId !== emp.id) {
        ...
        if (review.status !== performance_review_entity_1.ReviewStatus.SUBMITTED) {
            throw new common_1.ForbiddenException('Cannot edit a review that has already been reviewed by a manager');
        }
