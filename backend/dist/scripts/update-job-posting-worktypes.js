...
const job_posting_entity_1 = require("../src/job-postings/entities/job-posting.entity");
const dataSource = new typeorm_1.DataSource({
    ...
    entities: [job_posting_entity_1.JobPosting],
});
const WORK_TYPES = [job_posting_entity_1.WorkType.ONSITE, job_posting_entity_1.WorkType.HYBRID, job_posting_entity_1.WorkType.REMOTE];
function pickRandomWorkType() {
    return WORK_TYPES[Math.floor(Math.random() * WORK_TYPES.length)];
}
async function main() {
    await dataSource.initialize();
    const postingRepo = dataSource.getRepository(job_posting_entity_1.JobPosting);
...
//# sourceMappingURL=update-job-posting-worktypes.js.map